...
import { authenticateRequest, getClientIP, getSQL, logActivity, validateWasteSubmission } from './lib.js'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const storeName = String(body.storeName || 'BEKASI KP. BULU')
    ...
    const sql = getSQL()
    const result = await sql(`WITH claimed AS (INSERT INTO waste_submission_locks (business_date, shift, station) VALUES ($1::date, $2, $3) ON CONFLICT DO NOTHING RETURNING 1), inserted AS (INSERT INTO product_destructions (business_date, shift, store_name, kategori_induk, nama_produk, kode_produk, jumlah_produk, unit, metode_pemusnahan, alasan_pemusnahan, jam_tanggal_pemusnahan, paraf_qc_url, paraf_qc_name, paraf_manager_url, paraf_manager_name, dokumentasi_urls, submitted_by) SELECT $1::date, $2, $4, $3, product_name, product_code, product_quantity, product_unit, destruction_method, destruction_reason, destruction_time, $11, $12, $13, $14, $15, $16 FROM claimed CROSS JOIN UNNEST($5::text[], $6::text[], $7::int[], $8::text[], $9::text[], $10::text[], $17::text[]) AS item(product_name, product_code, product_quantity, product_unit, destruction_method, destruction_reason, destruction_time) RETURNING id), daily AS (INSERT INTO daily_records (business_date, shift, done, submitted_by, submitted_at) SELECT $1::date, $2, TRUE, $16, NOW() FROM claimed ON CONFLICT (business_date, shift) DO UPDATE SET done = TRUE, submitted_by = EXCLUDED.submitted_by, submitted_at = NOW() RETURNING id) SELECT COUNT(*)::int AS item_count FROM inserted`, [tanggal, shift, kategoriInduk, storeName, productList, kodeProdukList, jumlahProdukList, unitList, metodePemusnahanList, alasanPemusnahanList, String(body.parafQCUrl || ''), String(body.parafQCName), String(body.parafManagerUrl || ''), String(body.parafManagerName), (body.dokumentasiUrls as string[]).join('\n'), payload.sub, jamTanggalPemusnahanList])
