...
export default function MultiFileUpload({ files, onChange, maxFiles = 10 }: MultiFileUploadProps) {
  ...
  return <div className="space-y-3"><button type="button" onClick={() => inputRef.current?.click()} disabled={compressing} className="flex w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border bg-surface px-4 py-6 text-text-muted transition hover:border-brand-500 hover:text-brand-500 disabled:cursor-not-allowed disabled:opacity-50"><Upload size={20} /><span className="text-sm font-semibold">Upload Foto</span><span className="text-[11px]">JPG / PNG / WEBP • max {maxFiles} file • target ≤500 KB/foto</span>{compressing && <span className="text-[11px] text-warning-600 dark:text-warning-400">Lagi kompres gambar...</span>}</button><input ref={inputRef} type="file" multiple accept="image/*" className="hidden" onChange={(e) => void handleFiles(e.target.files)} />{statusText && <div role="status" className="rounded-lg border border-brand-200 bg-brand-50 px-3 py-2 text-xs text-brand-700 dark:border-brand-500/20 dark:bg-brand-500/10 dark:text-brand-400">{statusText}</div>}{error && <div role="alert" className="rounded-lg border border-error-200 bg-error-50 px-3 py-2 text-xs text-error-700 dark:border-error-500/20 dark:bg-error-500/10 dark:text-error-400">{error}</div>}{files.length > 0 && <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{files.map((file, index) => <div key={`${file.name}-${index}`} className="relative overflow-hidden rounded-xl border border-border bg-surface p-2 shadow-theme-xs"><div className="mb-2 flex aspect-square items-center justify-center overflow-hidden rounded-lg bg-surface-alt"><img src={previews[index]} alt={file.name} className="h-full w-full object-cover" /></div><button type="button" aria-label={`Hapus ${file.name}`} onClick={() => onChange(files.filter((_, i) => i !== index))} className="absolute right-3 top-3 rounded-full bg-gray-950/70 p-1 text-white"><X size={12} /></button><div className="flex items-center gap-1 text-[10px] text-text-muted"><ImageIcon size={10} /><span className="truncate">{file.name}</span></div><div className="mt-1 text-[10px] text-text-dim">{formatKb(file.size)}</div></div>)}</div>}</div>
}
