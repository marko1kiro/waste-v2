...
async function compressImage(file: File): Promise<File> {
  ...
  const output = webp?.type === 'image/webp' ? webp : await canvasBlob(canvas, 'image/jpeg')
  if (!output) throw new Error(`Gagal kompres ${file.name}.`)
  if (output.size > MAX_BYTES) throw new Error(`${file.name} melebihi 3 MB setelah kompresi.`)
  const extension = output.type === 'image/webp' ? 'webp' : 'jpg'
  return new File([output], `${file.name.replace(/\.[^.]+$/, '')}.${extension}`, { type: output.type === 'image/webp' ? 'image/webp' : 'image/jpeg', lastModified: Date.now() })
}
...
function canvasBlob(canvas: HTMLCanvasElement, type: string) { return new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, type, 0.8)) }
