...
function CodeBlock({ children, label = 'Code example' }: { children: string; label?: string }) {
  ...
  return <div className="relative my-5 overflow-hidden rounded-xl border border-gray-800 bg-gray-900 shadow-theme-md">
    <div className="flex items-center justify-between border-b border-gray-800 bg-gray-950 px-3 py-2 text-xs font-semibold text-gray-300"><span>{label}</span><button type="button" onClick={copy} className="flex items-center gap-1 rounded-md border border-gray-700 bg-gray-800 px-2 py-1 font-medium text-gray-200 transition hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-brand-500" aria-label={`Copy ${label}`}>{copied ? <Check size={14} aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}{copied ? 'Copied' : 'Copy'}</button></div>
    <pre className="overflow-x-auto p-4 text-sm leading-6 text-white"><code>{children}</code></pre>
  </div>
}

function Section({ id, title, children }: { id: string; title: string; children: ReactNode }) {
  return <section id={id} className="scroll-mt-6 border-t border-border py-10 first:border-t-0"><h2 className="mb-4 inline-block rounded-lg border border-border bg-surface px-3 py-1 text-2xl font-semibold text-text-primary shadow-theme-xs">{title}</h2>{children}</section>
}

export default function ApiDocs() {
  const [open, setOpen] = useState(false)
  const navigation = <ol className="space-y-1">{sections.map(([id, title]) => <li key={id}><a href={`#${id}`} onClick={() => setOpen(false)} className="block rounded-lg border border-transparent px-2 py-1 font-medium text-text-muted hover:bg-surface-alt hover:text-text-primary focus:outline-none focus:ring-2 focus:ring-brand-500">{title}</a></li>)}</ol>
  return <div className="min-h-dvh bg-background text-text-primary"><header className="border-b border-border bg-surface"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6"><Link href="/" className="rounded-lg bg-brand-500 px-3 py-2 font-semibold text-white shadow-theme-xs focus:outline-none focus:ring-2 focus:ring-brand-500">AWAS API</Link><Link href="/" className="rounded-lg border border-border bg-surface px-3 py-2 text-sm font-medium shadow-theme-xs focus:outline-none focus:ring-2 focus:ring-brand-500">Login / Home</Link></div></header>
    <div className="mx-auto grid max-w-7xl lg:grid-cols-[245px_minmax(0,1fr)]"><aside className="hidden border-r border-border bg-surface p-6 lg:block"><nav className="sticky top-6" aria-label="Documentation table of contents"><p className="mb-3 font-semibold">ON THIS PAGE</p>{navigation}</nav></aside><main className="min-w-0 px-4 py-8 sm:px-8 sm:py-12"><div className="mb-8 rounded-xl border border-border bg-surface p-5 shadow-theme-xs sm:p-8"><p className="mb-2 font-semibold uppercase text-brand-600 dark:text-brand-400">Public developer documentation</p><h1 className="text-4xl font-semibold leading-none sm:text-6xl">Waste integration API</h1><p className="mt-5 max-w-2xl text-lg font-medium text-text-muted">Create API keys with JWT, upload evidence, submit waste batches, manage items, retrieve daily data, generate PDFs.</p></div><div className="mb-6 lg:hidden"><button type="button" onClick={() => setOpen(!open)} aria-expanded={open} className="flex w-full items-center justify-between rounded-lg border border-border bg-surface p-3 font-semibold shadow-theme-xs"><span>Navigate documentation</span>{open ? <X aria-hidden="true" /> : <Menu aria-hidden="true" />}</button>{open && <nav className="mt-2 rounded-lg border border-border bg-surface p-3" aria-label="Documentation table of contents">{navigation}</nav>}</div>
    <Section id="quick-start" title="Quick start"><p>Production base URL: <code className="font-semibold">{baseUrl}</code>. JSON is used throughout; dates use <code>YYYY-MM-DD</code>.</p><CodeBlock label="Upload, then submit">{`export WASTE_API_KEY='awas_live_...'
