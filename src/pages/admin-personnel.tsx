...
export default function AdminPersonnel() {
  ...
  return (
    <div className="mx-auto max-w-7xl py-2">
      ...
      <div className="mb-5">
        <h1 className="text-xl font-black text-warning">Admin • Personnel</h1>
      ...
      <section className="mb-5 rounded-xl border-2 border-border bg-[#111] p-4 shadow-nb-md">
        <h2 className="mb-3 text-sm font-black text-text-primary">Tambah Personnel</h2>
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          <input placeholder="Short name" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} className="rounded-lg border-2 border-border bg-[#0d0d0d] px-3 py-2 text-sm text-text-primary" />
          <input placeholder="Full name" value={form.full_name} onChange={(e) => setForm((p) => ({ ...p, full_name: e.target.value }))} className="rounded-lg border-2 border-border bg-[#0d0d0d] px-3 py-2 text-sm text-text-primary" />
          <select value={form.role} onChange={(e) => setForm((p) => ({ ...p, role: e.target.value }))} className="rounded-lg border-2 border-border bg-[#0d0d0d] px-3 py-2 text-sm text-text-primary"><option value="qc">qc</option><option value="manager">manager</option></select>
          <button onClick={addPersonnel} disabled={creating} className="rounded-lg border-2 border-[#000] bg-warning px-4 py-2 text-sm font-black text-black shadow-nb-sm transition-all hover:-translate-x-px hover:-translate-y-px hover:shadow-nb-md active:translate-x-0 active:translate-y-0 active:shadow-none disabled:opacity-50">{creating ? 'Nyimpen...' : 'Tambah'}</button>
        </div>
      </section>
      ...
      <section className="rounded-xl border-2 border-border bg-[#111] p-4 shadow-nb-md">
        <div className="mb-2 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <h2 className="text-sm font-black text-text-primary">Daftar Personnel</h2>
          <div className="grid gap-2 md:grid-cols-3 xl:min-w-[640px]">
            <div className="flex items-center gap-2 rounded-lg border-2 border-border bg-[#0d0d0d] px-3 py-2 text-text-muted"><Search size={14} /><input placeholder="Cari nama / role" value={query} onChange={(e) => setQuery(e.target.value)} className="w-full bg-transparent text-sm text-text-primary outline-none" /></div>
            <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value as 'all' | 'qc' | 'manager')} className="rounded-lg border-2 border-border bg-[#0d0d0d] px-3 py-2 text-sm text-text-primary"><option value="all">Semua Role</option><option value="qc">QC</option><option value="manager">Manager</option></select>
            <select value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)} className="rounded-lg border-2 border-border bg-[#0d0d0d] px-3 py-2 text-sm text-text-primary"><option value="full_name">Sort: Full name</option><option value="name">Sort: Short name</option><option value="role">Sort: Role</option></select>
          </div>
        </div>

        <div className="mb-4 rounded-lg border border-border bg-[#0d0d0d] px-3 py-2 text-xs text-text-muted">
        ...
        {error && <div className="rounded-lg border border-danger/30 bg-danger/10 px-3 py-3 text-sm text-danger">Waduh gagal muat personnel. Coba refresh ya.</div>}
        {!isLoading && !error && rows.length === 0 && <div className="rounded-lg border border-border bg-[#0d0d0d] px-3 py-3 text-sm text-text-muted">Belum ada personnel.</div>}
          ...
          <>
            <div className="space-y-3 md:hidden">
              {rows.map((p) => {
                ...
                return (
                  <div key={p.id} className="rounded-xl border border-border bg-[#0d0d0d] p-3">
                    <div className="mb-3 grid gap-2">
                      <div>
                        <label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-[#666]">Short Name</label>
                        <input defaultValue={p.name} onBlur={(e) => { p.name = e.target.value }} className="w-full rounded border border-border bg-[#111] px-3 py-2 text-sm text-text-primary" />
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-[#666]">Full Name</label>
                        <input defaultValue={p.full_name} onBlur={(e) => { p.full_name = e.target.value }} className="w-full rounded border border-border bg-[#111] px-3 py-2 text-sm text-text-primary" />
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-[#666]">Role</label>
                        <select defaultValue={p.role} onChange={(e) => { p.role = e.target.value as 'qc' | 'manager' }} className="w-full rounded border border-border bg-[#111] px-3 py-2 text-sm text-text-primary"><option value="qc">qc</option><option value="manager">manager</option></select>
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-[#666]">Signature</label>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => savePersonnel({ ...p })} disabled={isSaving || isUploading || isDeleting} className="flex-1 rounded-lg border-2 border-primary/40 bg-primary/10 px-3 py-2 text-xs font-black text-primary shadow-nb-sm transition-all hover:-translate-x-px hover:-translate-y-px hover:shadow-nb-md active:translate-x-0 active:translate-y-0 active:shadow-none disabled:opacity-50">{isSaving ? 'Nyimpen...' : 'Simpan'}</button>
                      <button onClick={() => setDeleting(p)} disabled={isSaving || isUploading || isDeleting} className="flex-1 rounded-lg border-2 border-danger/40 bg-danger/10 px-3 py-2 text-xs font-black text-danger shadow-nb-sm transition-all hover:-translate-x-px hover:-translate-y-px hover:shadow-nb-md active:translate-x-0 active:translate-y-0 active:shadow-none disabled:opacity-50">{isDeleting ? '...' : 'Hapus'}</button>
                    </div>
                  </div>
                )
              })}
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {rows.map((p) => {
                    ...
                    const isDeleting = deletingId === p.id
                    return (
                      <tr key={p.id} className="border-t border-border bg-[#0d0d0d] align-top">
                        <td className="px-3 py-3"><input defaultValue={p.name} onBlur={(e) => { p.name = e.target.value }} className="w-full rounded border border-border bg-[#111] px-2 py-1 text-text-primary" /></td>
                        <td className="px-3 py-3"><input defaultValue={p.full_name} onBlur={(e) => { p.full_name = e.target.value }} className="w-full rounded border border-border bg-[#111] px-2 py-1 text-text-primary" /></td>
                        <td className="px-3 py-3"><select defaultValue={p.role} onChange={(e) => { p.role = e.target.value as 'qc' | 'manager' }} className="rounded border border-border bg-[#111] px-2 py-1 text-text-primary"><option value="qc">qc</option><option value="manager">manager</option></select></td>
                        ...
                        <td className="px-3 py-3">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => savePersonnel({ ...p })} disabled={isSaving || isUploading || isDeleting} className="rounded-lg border-2 border-primary/40 bg-primary/10 px-3 py-1.5 text-xs font-black text-primary shadow-nb-sm transition-all hover:-translate-x-px hover:-translate-y-px hover:shadow-nb-md active:translate-x-0 active:translate-y-0 active:shadow-none disabled:opacity-50">{isSaving ? 'Nyimpen...' : 'Simpan'}</button>
                            <button onClick={() => setDeleting(p)} disabled={isSaving || isUploading || isDeleting} className="rounded-lg border-2 border-danger/40 bg-danger/10 px-3 py-1.5 text-xs font-black text-danger shadow-nb-sm transition-all hover:-translate-x-px hover:-translate-y-px hover:shadow-nb-md active:translate-x-0 active:translate-y-0 active:shadow-none disabled:opacity-50">{isDeleting ? '...' : 'Hapus'}</button>
                          </div>
                        </td>
                      </tr>
                    )
